...
| `api`                                          | Node 20 + Express BFF：`GET /api/indices/catalog`（可选指数）、`GET /api/indices/:code/market/rt`（活体行情）                             |
...
- **大盘活体（仅此接口）**：`GET /api/indices/{code}/market/rt`——**仅此指数最新一批成分**若在 `quotes_rt` 里有快照则走 rt（缺则用日线补缺）；否则全部走 `quotes_daily`；rt/日线聚合均先限定这批成分标的。
  - 新库/升级请按序执行 [db/migrations](db/migrations)（`001`…`009` 等，见目录内文件名）；`docker compose` 的 `db` 镜像在构建时把 `db/migrations` 复制进 `docker-entrypoint-initdb.d`，**仅在新数据目录首次初始化**时按文件名顺序执行其中全部 `.sql`；修改 SQL 后需 `docker compose build db`（或 `up --build`）。已有库请按需手动执行尚未应用的脚本。
...
- HTTP API（仅此入口对外时走同源前缀）：`/api/…`，例如 [http://localhost/api/indices/catalog](http://localhost/api/indices/catalog)
