...
自由流通股本（`free_share`，万股）来自 `**daily_basic`** [doc 32](https://tushare.pro/document/2?doc_id=32)，写入 `share_premarket`；`quotes_* .circ_mv` = `free_share×10000×close`（元），与 Tushare `circ_mv`(万元) 不同口径。
...
- **收盘**：工作日 **16:15** 任务：**trade_cal(doc 26)** 同步 SSE 日历 → `index_weight` → 申万 → `daily_basic`（**自由流通股本 `free_share`**）→ **daily** 更新 `quotes_daily` 并裁剪约 30 交易日 → `**TRUNCATE quotes_rt`**（日线已落库，清空实时表）。
